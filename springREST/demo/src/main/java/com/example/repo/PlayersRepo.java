package com.example.repo;

import java.util.List;

import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.data.jpa.repository.Query;
import org.springframework.stereotype.Repository;

import com.example.entity.Players;

@Repository
public interface PlayersRepo extends JpaRepository<Players, Integer>{

    @Query(value="select * from players where position = ?1",nativeQuery=true)
    public List<Players> findPlayerByPosition(String position);
    
    @Query(value="select * from players where name LIKE %?1%",nativeQuery=true)
    public List<Players> findPlayerByName(String name);

    @Query(value="select * from players where stats_id = ?1",nativeQuery=true)
    public Players findPlayerByStatId(Integer statId);
    
}
